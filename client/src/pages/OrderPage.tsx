...
const OrderPage = () => {
  ...
  const fetchOrder = async () => {
    try {
      const { data } = await axios.get(`${outletService}/api/order/${orderId}`, {
        headers: {
  ...
  useEffect(() => {
    ...
    socket.on("order:picked_up", onOrderUpdate);
    socket.on("order:delivered", onOrderUpdate);

    return () => {
      socket.off("order:update", onOrderUpdate);
      socket.off("order:rider_assigned", onOrderUpdate);
      socket.off("order:picked_up", onOrderUpdate);
      socket.off("order:delivered", onOrderUpdate);
    };
  }, [socket]);

  // make socket room for this order
  // useEffect(() => {
  ...
  //   };
  // }, [socket, orderId]);
  // not needed as we are joining room with user:userId not user:orderId and backend 
  // is emitting order and rider location updates to user room so no additonal room is needed 
  // also we have not set up listner for a "join" event in backend
