...
const RiderDashboard = () => {
  ...
  const [profile, setProfile] = useState<RiderInterface | null>(null);
  ...
  useEffect(() => {
    if (user === undefined) return; // Wait for user data to be available
  ...
  const handleSubmit = async () => {
    if (!aadhaarNumber || !phoneNumber || !image || !drivingLicenseNumber) {
    ...
    if (!navigator.geolocation) {
      console.log("Location Access Requireddddddddddd");
    ...
    try {
      // 2. Check if location permission is denied
      ...
      // Fallback for older browsers that don't support permissions.query
      console.warn("Permissions API not supported, proceeding to geolocation check.");
    }
    ...
    (error) => {
      // 3. Handles cases where location is globally turned off on the device
  ...
  return (
    <div className="space-y-4">
        ...
        <div className="rounded-xl bg-white p-4 shadow space-y-3">
          ...
          <div>
            <p className="text-blue-400 text-sm">
              Please be within a 500 m radius of any outlet (which we call a
      ...
      {currentOrder && (
        <div className="mx-auto max-w-md px-4 space-y-4 pb-10">
          <RiderCurrentOrder
            order={currentOrder}
