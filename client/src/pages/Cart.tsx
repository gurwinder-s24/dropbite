...
import type { CartInterface, MenuItemInterface, OutletInterface } from "../types";
...
const Cart = () => {
  ...
  const clearCart = async () => {
    ...
    try {
      ...
      await axios.delete(`${outletService}/api/cart/clear`, {
        headers: {
          Authorization: `Bearer ${localStorage.getItem("token")}`,
        },
      });
      ...
      <div className="rounded-xl bg-white p-4 shadow-sm space-y-3">
        ...
        <button
          ...
          className={`mt-3 w-full rounded-lg bg-[#E23744] py-3 text-sm font-semibold text-white hover:bg-red-800 ${
            !outlet.isOpen ? "opacity-50 cursor-not-allowed" : ""
          }`}
          disabled={!outlet.isOpen}
        >
          {!outlet.isOpen ? "Outlet is Closed" : "Proceed to Checkout"}
        </button>
