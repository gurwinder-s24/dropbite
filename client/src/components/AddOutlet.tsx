...
interface props {
  fetchMyOutlet: () => Promise<void>;
}

const AddOutlet = ({ fetchMyOutlet }: props) => {
  ...
  const handleSubmit = async () => {
    ...
    if (!location) {
      alert("One time Location access is required to set address of the outlet");
      return;
    }
    ...
    try {
      setSubmitting(true);
      await axios.post(`${outletService}/api/outlet/new`, formData, {
  ...
  return (
    <div className="min-h-screen bg-gray-50 px-4 py-6">
      <div className="mx-auto max-w-lg rounded-xl bg-white p-6 shadow-sm space-y-5">
        <h1 className="text-xl font-semibold">Add Your Outlet</h1>
        <input
          type="text"
        ...
        <label className="flex cursor-pointer items-center gap-3 rounded-lg border p-4 text-sm text-gray-600 hover:bg-gray-50">
          ...
          {image ? image.name : "Upload Outlet image"}
          <input
            type="file"
            accept="image/*"
            hidden
        ...
        >
          {submitting ? "Submitting..." : "Add Outlet"}
        </button>
      </div>
    </div>
  );
