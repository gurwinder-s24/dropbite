...
const OrderCard = ({ order, onStatusUpdate }: props) => {
  ...
  const resetTimerForButtonVisibility = () => {
  ...
  useEffect(() => {
    ...
    return resetTimerForButtonVisibility();
  }, [order.status]);

  const updateStatus = async (status: string) => {
    if (order.status === "ready_for_rider" && status === "ready_for_rider") {
      try{
        setRetryVisible(false);
        await axios.post(
          `${outletService}/api/order/${order._id}/notify_riders`,
          {},
          { headers: { Authorization: `Bearer ${localStorage.getItem("token")}` } }
        );
        
        toast.success("Riders notified again");
        resetTimerForButtonVisibility();
      } catch (error: any) {
    ...
    try {
      ...
      setRetryVisible(false);
      await axios.put(
        `${outletService}/api/order/${order._id}`,
        { status },
        { headers: { Authorization: `Bearer ${localStorage.getItem("token")}` } }
      );
  ...
  return (
    <div className="rounded-xl bg-white p-4 shadow-sm space-y-3">
      ...
      <div className="flex justify-between text-sm font-medium pr-3">
        <span>Total</span>
