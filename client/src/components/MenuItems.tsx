...
const MenuItems = ({ items, fetchMenuItems, isSeller }: MenuItemsProps) => {
  ...
  const handleDelete = async (itemId: string) => {
    ...
    try {
      await axios.delete(`${outletService}/api/item/${itemId}`, {
  ...
  const toggleAvailiblity = async (itemId: string) => {
    try {
      const { data } = await axios.put(
        `${outletService}/api/item/status/${itemId}`,
        {},
        {
          headers: { Authorization: `Bearer ${localStorage.getItem("token")}` },
        }
  ...
  const addToCart = async (outletId: string, itemId: string) => {
    try {
      setLoadingItemId(itemId);

      const { data } = await axios.post(
        `${outletService}/api/cart/add`,
        {
          outletId,
          itemId,
        },
              ...
              <div className="flex items-center justify-between ">
                ...
                {!isSeller && (
                  <button
                    ...
                    onClick={() => addToCart(item.outletId , item._id)}
