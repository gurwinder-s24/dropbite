...
const PendingRiderCard = ({ rider, onVerify } : { rider: any; onVerify: () => void }) => {
  const verify = async () => {
    try {
      await axios.patch(
  ...
  return (
    <div className="rounded-xl bg-white p-4 shadow space-y-2">
      ...
      <p className="text-sm text-gray-500">aadhar {rider.aadhaarNumber}</p>
      <p className="text-sm text-gray-500">dl {rider.drivingLicenseNumber}</p>
