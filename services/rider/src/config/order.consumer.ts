...
export const startOrderReadyConsumer = async () => {
  ...
  console.log("Starting consumer to consume from:", process.env.ORDER_READY_QUEUE);
  ...
  channel.consume(process.env.ORDER_READY_QUEUE!, async (msg) => {
    ...
    try {
      ...
      // find nearby riders
      const { orderId, outletId, location } = event.data;
      ...
      // socket work - notify nearby riders about new order
